# Промт для Claude Cowork: ссылки под видео на YouTube-канале «Мужская среда»

Скопируй всё, что ниже разделителя, в Cowork. Промт самодостаточный — доступ к репозиторию
не нужен, нужен только доступ к каналу.

---

Ты помогаешь с внешними упоминаниями для сайта `sreda.men` — это закрытое онлайн-сообщество
для мужчин 25+, встречи в Zoom раз в две недели. Ведущие: Влад (гештальт-терапевт) и Женя
(психиатр-нарколог).

## Задача

Открой канал https://www.youtube.com/@men-env — там около 18 видео, плюс посты во вкладке
«Сообщество». Для **каждого** видео и каждого поста подбери одну статью из каталога ниже,
которая ближе всего по теме, и составь готовый к вставке блок для описания.

Смысл работы: сайту 4 месяца, у него ноль внешних ссылок, и Google его пока не индексирует.
Восемнадцать описаний — это восемнадцать **разных** точек входа вглубь блога. Поэтому одинаковая
ссылка во всех описаниях обесценивает всю затею: каждое описание ведёт на свою статью.
...
7. **Распределение анкоров по всем 18 описаниям вместе:**
   - **8 брендовых** — «Мужская среда», «сообщество Мужская среда», `sreda.men`;
   - **8 безанкорных** — «вот здесь», «в этой статье», «подробный разбор», голый URL;
   - **2 с вхождением ключа** — берутся из нейтральных: «как проходит мужская группа»,
     «мужской круг», «онлайн-сообщество для мужчин».

   **Отдельный запрет:** анкор «мужская группа поддержки» использовать нельзя, даже если статья
   именно под этот запрос. Статья может отвечать на запрос, не называя так себя, а анкор назвал бы
   сообщество «группой поддержки» прямо — это нарушает позиционирование проекта.
8. **Если для видео нет подходящей статьи — скажи прямо.** Не подгоняй далёкую по теме.

## Формат ответа

Таблица:

| № | Название видео (дословно) | Статья (slug) | URL | Анкор | Тип анкора | Текст для описания |
|---|---|---|---|---|---|---|

В колонке «Текст для описания» — одна–две живые фразы со встроенной ссылкой, готовые к вставке.
Без канцелярита, без «в данной статье вы узнаете».

После таблицы дай:
- **сводку по анкорам**: сколько брендовых, безанкорных, с вхождением — и сходится ли с 8/8/2;
- **список видео, для которых статьи не нашлось**, если такие есть;
- **какие темы каталога остались не использованы** — это подсказка, о чём стоит снять следующий ролик.
